...
def auth():
    ...
    if session.get('oauth_token') is True:
        ...
        user = Usercat.query.fliter_by(id=load_profile['id']).one()
        login_user(user)
...
def callback():
    ...
    if Usercat.query.filter_by(id=load_profile['id']).first() is not None:
        user = Usercat.query.filter_by(id=load_profile['id']).one()
        login_user(user)

    # If user is not in the database make a new entry
    else:
        newUser = Usercat(username=load_profile['login'],
                          id=load_profile['id'],
...
# --DEVELOPER LOGIN-REMOVE BEFORE LAUNCH-DEVELOPER LOGIN-REMOVE BEFORE LAUNCH--
        if 'dev' in request.form:
            devuser = Usercat.query.filter_by(id=8172146).first()
            login_user(devuser)
            return redirect('catalog')
# --DEVELOPER LOGIN-REMOVE BEFORE LAUNCH-DEVELOPER LOGIN-REMOVE BEFORE LAUNCH--
